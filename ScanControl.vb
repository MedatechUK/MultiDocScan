...
Public Class ScanControl
    ...
    Public Shared Sub process(ByVal f As ScanDocument)
        ...
        Dim D As New ScanControl.ScanSettings()
        Dim cDoc As New Dictionary(Of Integer, iDocument)
        ...
        For Each img As System.Drawing.Image In imgs
            Try
                Console.Write("Decoding image barcode...")
                Dim decodedBarcode As Result = decoder.Decode(img, bformats)
                Console.WriteLine( _
                    String.Format( _
                        "Read: {0}", _
                        decodedBarcode.Text _
                    ) _
                )
                With cDoc
                    .Add(.Keys.Count, New iDocument(decodedBarcode.Text))
                    .Item(.Keys.Count - 1).Add(img)
                End With

            Catch e1 As DTI.ImageMan.Barcode.NotFoundException
                Console.WriteLine( _
                    String.Format( _
                        "No barcode, adding image to {0}.", _
                        cDoc(cDoc.Count - 1).scan_doc_code _
                    ) _
                )
                With cDoc
                    .Item(.Keys.Count - 1).Add(img)
        ...
        For Each k As Integer In cDoc.Keys

            Console.WriteLine( _
                String.Format( _
                    "Writing PDF... {0}.", _
                    cDoc(k).scan_doc_code _
                ) _
            )
            ...
            For Each img As System.Drawing.Image In cDoc(k)
                Console.WriteLine( _
                    String.Format( _
                        "Adding page {0} of {1}.", _
                        i + 1, _
                        cDoc(k).Count _
                    ) _
                )
                ...
                Dim imgx As XImage = XImage.FromGdiPlusImage(img)
                xgr.DrawImage(imgx, 0, 0)
                i += 1
            ...
            Do
                i += 1
                    ...
                    Path.Combine( _
                        Path.Combine( _
                            f.doc_dir, _
                            cDoc(k).scan_type_code _
                        ), _
                        String.Format( _
                            "{0}_{1}_{2}.pdf", _
                            cDoc(k).scan_doc_code, _
                            Now.ToString("yyyyMMdd"), _
                            Strings.Right("0000" & i, 4)) _
                        ) _
                    )
            ...
            Loop Until Not fi.Exists

            Console.WriteLine( _
                String.Format("Writing PDF to ... [{0}]", _
                    fi.FullName _
                ) _
            )
            doc.Save(fi.FullName)
            Console.WriteLine( _
                String.Format("Writing database log ... ", _
                    fi.FullName _
                ) _
            )
            write_log(f.doc_dir, fi.Name, f.doc_date, cDoc(k).scan_type_code, cDoc(k).scan_doc_code, f.batch_no, D.ConStr)

        Next

    End Sub

    Public Shared Sub process2(ByVal f As ScanDocument)
